...
func after_each():
	_combat.free()
	# Player.arrival_snapshot est un singleton partage par toute la suite
	# (cf player.gd) -- le remettre a son defaut evite qu'un test ici ne
	# fausse peut_abandonner() dans un autre fichier (meme piege que
	# Sounder.player.stream, cf test_success_popup.gd).
...
func test_bouton_abandonner_a_une_legende_visible_pas_seulement_un_tooltip():
	Player.arrival_snapshot = {"retour": 5, "pv": 10, "cha": 2, "items": []}
	_start()
	var found = false
	for child in _combat._abandon_button.get_children():
		for grand_child in child.get_children():
			if grand_child is Label and grand_child.text == "Abandonner":
				found = true
	assert_true(found, "le bouton abandonner doit porter une legende visible en permanence")


func test_bouton_abandonner_desactive_sur_le_tout_premier_noeud_de_la_session():
	# retour=-1 : rien ou revenir, cf player.gd::arrival_snapshot.
	Player.arrival_snapshot = {"retour": -1, "pv": 0, "cha": 0, "items": []}
	_start()
	assert_true(_combat._abandon_button.disabled)


func test_bouton_abandonner_actif_quand_un_retour_existe():
	Player.arrival_snapshot = {"retour": 5, "pv": 10, "cha": 2, "items": []}
	_start()
	assert_false(_combat._abandon_button.disabled)


func test_abandonner_ferme_le_panneau_et_emet_le_signal():
	Player.arrival_snapshot = {"retour": 5, "pv": 10, "cha": 2, "items": []}
	_start()
	watch_signals(_combat)
	_combat._on_abandon_pressed()
	assert_signal_emitted(_combat, 'combat_abandonne')
	assert_false(_combat.visible)
	assert_false(Player.in_combat)


func test_abandonner_ne_fait_rien_si_deja_desactive():
	Player.arrival_snapshot = {"retour": -1, "pv": 0, "cha": 0, "items": []}
	_start()
	watch_signals(_combat)
	_combat._on_abandon_pressed()
	assert_signal_not_emitted(_combat, 'combat_abandonne')
	assert_true(_combat.visible, "sans retour possible, le panneau doit rester ouvert")
...
func test_debrouillard_relance_automatiquement_et_garde_le_meilleur_de():
	_start({"peut_relancer_attaque": true})
	var t = await _combat._play_turn(2, null, 5)  # premier de=2, relance=5
	assert_eq(t.attack_die_roll, 5, "le de final affiche/anime doit etre le meilleur des deux")


func test_sans_debrouillard_le_de_de_relance_fourni_est_ignore():
	_start()  # peut_relancer_attaque=false (defaut)
	var t = await _combat._play_turn(2, null, 6)
	assert_eq(t.attack_die_roll, 2, "pas DEBROUILLARD : la relance fournie ne doit jamais s'appliquer")


func test_tag_ecart_hors_bareme_visible_seulement_quand_lecart_reel_depasse_la_table():
	# _start() sans opts : hab_billy=9, hab_adversaire=5, ecart brut 4 --
	# largement dans la table, aucun tag.
	_start()
	assert_false(_combat._ecart_plafonne_tag.visible)

	# Avec pyro_bonus=4 (meme scenario que test_bonus_pyro_affiche_seulement_si_non_nul,
	# "GUERRIER+Pyro-Barbare" reel) : ecart brut 13-5=8, au-dela de la
	# derniere ligne couverte (+7) -- le tag doit s'afficher, MAIS le combat
	# se joue normalement (cf combat.gd, is_auto_win() n'est volontairement
	# pas cable a is_over()/get_winner() : ca casserait ce scenario reel deja
	# teste et voulu tel quel, cf test_full_combat_guerrier_epee_morgenstern_marmite_vs_guerriers_orcs).
	_start({"pyro_bonus": 4})
	assert_true(_combat._ecart_plafonne_tag.visible)
	assert_false(_combat.is_resolved(), "un ecart hors bareme ne termine pas le combat tout seul")
