...
# =============================================================================
# is_auto_win() / is_ecart_plafonne() -- ecart brut au-dela de la table.
# =============================================================================

func test_ecart_dans_la_table_nest_ni_auto_win_ni_plafonne():
	var combat = CombatScript.new(5, 5, 20, 20)  # ecart brut 0
	assert_false(combat.is_auto_win())
	assert_false(combat.is_ecart_plafonne())


func test_ecart_brut_exactement_7_nest_pas_encore_auto_win():
	var combat = CombatScript.new(12, 5, 20, 20)  # ecart brut 7, derniere ligne couverte
	assert_eq(combat.get_ecart_brut(), 7)
	assert_false(combat.is_auto_win(), "7 est encore dans la table, pas au-dela")
	assert_false(combat.is_ecart_plafonne())


func test_ecart_brut_au_dela_de_7_est_signale_mais_ne_saute_pas_le_de():
	# is_auto_win()/is_ecart_plafonne() sont des signaux PURS pour l'UI --
	# ils ne court-circuitent PAS is_over()/get_winner(). Verifie contre un
	# vrai scenario deja teste et voulu (GUERRIER+Pyro-Barbare, ecart brut 8,
	# "DOMINATION") : sauter le de aurait casse ce comportement etabli, cf
	# test_full_combat_guerrier_epee_morgenstern_marmite_vs_guerriers_orcs.
	var combat = CombatScript.new(20, 5, 20, 20)  # ecart brut 15
	assert_eq(combat.get_ecart_brut(), 15)
	assert_true(combat.is_auto_win())
	assert_true(combat.is_ecart_plafonne())
	assert_false(combat.is_over(), "un ecart hors bareme ne termine pas le combat tout seul")
	assert_null(combat.get_winner())
	combat.play_turn(1)
	assert_eq(combat.tour, 1, "le tour se joue normalement, seule l'UI est prevenue via is_auto_win()")


func test_ecart_brut_sous_moins_7_est_plafonne_mais_pas_une_defaite_automatique():
	# Volontairement ASYMETRIQUE (cf combat.gd::is_auto_win) : un ecart tres
	# defavorable est signale (is_ecart_plafonne) mais ne termine PAS le
	# combat tout seul -- rien de source pour une "defaite automatique".
	var combat = CombatScript.new(5, 20, 20, 20)  # ecart brut -15
	assert_eq(combat.get_ecart_brut(), -15)
	assert_false(combat.is_auto_win())
	assert_true(combat.is_ecart_plafonne())
	assert_false(combat.is_over(), "pas de defaite automatique, seulement un signal a l'UI")


# =============================================================================
# DEBROUILLARD -- relance automatique de l'attaque, garde le meilleur.
# =============================================================================

func test_debrouillard_garde_le_meilleur_des_deux_des_quand_la_relance_est_superieure():
	var combat = CombatScript.new(5, 5, 20, 20, {"peut_relancer_attaque": true})
	var t = combat.play_turn(2, null, 5)  # premier de=2, relance=5 -> garde 5
	assert_eq(t.attack_die_roll, 5)


func test_debrouillard_garde_le_meilleur_des_deux_des_quand_le_premier_est_superieur():
	var combat = CombatScript.new(5, 5, 20, 20, {"peut_relancer_attaque": true})
	var t = combat.play_turn(6, null, 1)  # premier de=6 (le meilleur possible), relance=1
	assert_eq(t.attack_die_roll, 6, "relancer ne peut jamais faire pire que garder le premier de")


func test_sans_le_pouvoir_debrouillard_la_relance_fournie_est_ignoree():
	var combat = CombatScript.new(5, 5, 20, 20)  # peut_relancer_attaque=false par defaut
	var t = combat.play_turn(2, null, 6)  # relance=6 fournie mais sans le pouvoir, jamais consultee
	assert_eq(t.attack_die_roll, 2, "pas DEBROUILLARD : le premier de est le de final, la relance est ignoree")
