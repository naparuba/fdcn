...
func test_get_id():
	assert_eq(BookData.get_node(14).get_id(), 14.0)


func test_combat_simple_dict():
	var node = BookData.get_node(14)
...
func test_combat_as_list_returns_first_entry():
	...
	var node = BookData.get_node(276)
	assert_true(node.is_combat())
...
func test_non_combat_node():
	var node = BookData.get_node(1)
	assert_false(node.is_combat())


func test_aquire_and_remove():
	assert_eq(BookData.get_node(112).get_aquire(), ["PALAIS DES PLAISIRS D'YTIA"])
	assert_eq(BookData.get_node(166).get_remove(), ["PHILTRE D'AMOUR"])
	assert_eq(BookData.get_node(1).get_aquire(), [])
	assert_eq(BookData.get_node(1).get_remove(), [])


func test_label():
	assert_eq(BookData.get_node(10).get_label(), 'Tour nord')
	assert_null(BookData.get_node(1).get_label())


func test_secret_flag():
	assert_true(BookData.get_node(13).get_secret())
	assert_false(BookData.get_node(1).get_secret())


func test_success_field():
	assert_eq(BookData.get_node(26).get_success(), 'POLIR-LANCE')
	assert_null(BookData.get_node(1).get_success())


func test_sons_list():
	# les ids viennent du JSON compile -> ce sont des float (184.0), pas des int
	var sons = BookData.get_node(10).get_sons()
...
func test_stats_and_stats_cond():
	assert_eq_deep(BookData.get_node(128).get_stats(), {'end': 1.0})
	assert_eq(BookData.get_node(128).get_stats_cond(), [])
	assert_eq(len(BookData.get_node(126).get_stats_cond()), 4)
...
func test_ending_fields_on_a_real_ending_node():
	var node = BookData.get_node(163)  # bad ending, ending_id="TULIPES"
