...
# Emis quand le joueur abandonne totalement le combat (pas juste un tour --
# cf _on_abandon_pressed()) : main.gd restaure alors Player.arrival_snapshot
# et revient au chapitre precedent. Aucune reference a main.gd depuis cet
# ecran (cf en-tete de fichier, tout passe par autoload/signal) -- la
# connexion se fait cote main.tscn, comme generic_popup_accept.
signal combat_abandonne()

const COL_NAVY = Palette.COL_NAVY
const COL_PAPER = Palette.COL_PAPER
const COL_CARD = Palette.COL_CARD
const COL_CARD_ALT = Palette.COL_CARD_ALT
const COL_TEAL = Palette.COL_TEAL
const COL_CORAIL = Palette.COL_CORAIL
const COL_CYAN = Palette.COL_CYAN
const COL_GOLD = Palette.COL_GOLD
const COL_ORANGE = Palette.COL_ORANGE  # #C27200, dice/evenements periodiques (SPEC #8)
const COL_INK = Palette.COL_INK
const COL_INK_SOFT = Palette.COL_INK_SOFT
...
func start_combat(enemy_name: String, hab_billy: int, hab_adversaire: int,
	...
	self._ecart_plafonne_tag.visible = self._controller.is_ecart_plafonne()
...
func _build_ui():
	...
	# Previent quand l'ecart d'Habilete REEL depasse ce que couvre la Table
	# des Situations (dans un sens ou l'autre) -- sinon la situation
	# affichee (bornee a la derniere ligne couverte) donne l'impression
	# d'un bug plutot que d'etre juste plus extreme que ca (cf
	# combat.gd::is_ecart_plafonne()).
	self._ecart_plafonne_tag = self._make_tag_label("Écart hors barème", COL_CARD_ALT, COL_INK_SOFT)
	self._ecart_plafonne_tag.visible = false
	player['tags_box'].add_child(self._ecart_plafonne_tag)
	...
	# Abandon TOTAL du combat (pas juste le dernier tour, cf _undo_button
	# ci-dessus) -- complementaire, pas remplaçant (PR16_RECOVERY_PLAN.md
	# §9). Grise sur le tout premier noeud de la session, ou "abandonner"
	# n'a nulle part ou revenir (cf peut_abandonner()).
	self._abandon_button = Button.new()
	self._abandon_button.custom_minimum_size = Vector2(60, 52)
	self._abandon_button.disabled = true
	self._style_solid_button(self._abandon_button, COL_CARD, COL_CORAIL)
	self._abandon_button.pressed.connect(self._on_abandon_pressed)
	action_row.add_child(self._abandon_button)
	var abandon_label_box = VBoxContainer.new()
	abandon_label_box.mouse_filter = Control.MOUSE_FILTER_IGNORE
	abandon_label_box.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	abandon_label_box.alignment = BoxContainer.ALIGNMENT_CENTER
	self._abandon_button.add_child(abandon_label_box)
	var abandon_icon = Label.new()
	abandon_icon.text = "✕"
	abandon_icon.add_theme_font_size_override("font_size", 18)
	abandon_icon.add_theme_color_override("font_color", COL_CORAIL)
	abandon_icon.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	abandon_label_box.add_child(abandon_icon)
	var abandon_caption = Label.new()
	abandon_caption.text = "Abandonner"
	abandon_caption.add_theme_font_size_override("font_size", 9)
	abandon_caption.add_theme_color_override("font_color", COL_INK_SOFT)
	abandon_caption.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	abandon_label_box.add_child(abandon_caption)
...
func _style_solid_button(button: Button, bg: Color, fg: Color):
	Palette.style_solid_button(button, bg, fg, {
		"corner_radius": 12,
		"shadow_color": Color(0.05, 0.08, 0.12, 0.12),
		"shadow_size": 5,
	})
...
func _refresh_bars():
	...
	self._abandon_button.disabled = !self._controller.peut_abandonner()
...
# et reproductibles (forcer attack_die/esquive_die/attack_die_relance) --
# memes parametres que combat.gd::play_turn(), null = de reel/aleatoire
# (comportement du bouton). attack_die_relance ne sert qu'au DEBROUILLARD.
func _play_turn(attack_die = null, esquive_die = null, attack_die_relance = null):
	if self._rolling or self.is_resolved():
		return null
	self._rolling = true
	self._roll_button.disabled = true

	var nouveau_tour = self._controller.jouer_tour(attack_die, esquive_die, attack_die_relance)
...
# Abandon TOTAL du combat (§9) -- ferme le panneau comme _on_continue_pressed()
# le fait pour une resolution normale, mais sans jamais passer par
# _show_resolution() : il n'y a pas de vainqueur, le combat n'a pas eu
# lieu du point de vue du livre. main.gd restaure Player.arrival_snapshot
# et revient au chapitre precedent en reponse au signal (cf en-tete de
# fichier -- aucune reference a main.gd depuis cet ecran).
func _on_abandon_pressed():
	if !self._controller.peut_abandonner():
		return
	self.visible = false
	Player.in_combat = false
	self.combat_abandonne.emit()
