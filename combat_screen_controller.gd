...
# Vrai quand l'ecart reel depasse ce que couvre la Table des Situations
# (dans un sens ou l'autre) -- cf combat.gd::is_ecart_plafonne().
func is_ecart_plafonne() -> bool:
	return self.combat.is_ecart_plafonne()
...
# Vrai s'il existe un chapitre "d'avant" vers lequel abandonner le combat --
# faux sur le tout premier noeud de la session (aucun "retour" possible),
# cf player.gd::arrival_snapshot et PR16_RECOVERY_PLAN.md §9.
func peut_abandonner() -> bool:
	return Player.arrival_snapshot.get("retour", -1) != -1
...
# Joue un tour complet (les trois des si non fournis -- utile pour des
# tests deterministes ; attack_die_relance ne sert qu'au DEBROUILLARD, cf
# combat.gd::play_turn()). Retourne l'EtatTour empile, ou null si le
# combat est deja termine.
func jouer_tour(attack_die = null, esquive_die = null, attack_die_relance = null):
	if self.is_resolved():
		return null
	return self.combat.play_turn(attack_die, esquive_die, attack_die_relance)
