...
##
## Toute règle vit dans le moteur (`autoload/combat_engine.gd`). Ici on lit son état, on
## lui transmet les décisions du joueur, et on peint. La séparation est ce qui rend les
## règles testables sans interface.
##
## TROIS ÉTATS, un seul panneau :
...
func _set_etat(etat: int) -> void:
	...
	# calculer : ni dés, ni esquive, ni écart.
...
## calculs disent que tu es mort », jamais « tu n'as pas le droit ».
