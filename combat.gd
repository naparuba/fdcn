...
# La Table des Situations elle-meme (degats par diff/jet, cout de fuite,
# nom de palier) vit desormais dans combat-table.json, chargee par
# l'autoload CombatTable (cf PR16_RECOVERY_PLAN.md §5 -- un seul fichier a
# auditer/corriger plutot que 3 constantes GDScript eparpillees). Valeurs
# numeriques inchangees.
...
static func get_tier_name(hab_billy, hab_adversaire):
	return CombatTable.get_tier_name(clamp_diff(hab_billy - hab_adversaire))


static func get_fuite_cost(hab_billy, hab_adversaire):
	return CombatTable.get_fuite_cost(clamp_diff(hab_billy - hab_adversaire))
...
static func resolve_round(hab_billy, hab_adversaire, die_roll):
	...
	var pair = CombatTable.get_pair(diff, die_roll)
...
var peut_relancer_attaque = false  # vrai pour un Billy DEBROUILLARD
...
#   PAYSAN, applique apres l'Armure), peut_relancer_attaque (vrai pour un
#   Billy DEBROUILLARD, cf play_turn()), modificateurs (Array d'instances
...
func _init(p_hab_billy, p_hab_adversaire, p_pv_billy, p_pv_adversaire, opts = {}):
	...
	self.peut_relancer_attaque = opts.get('peut_relancer_attaque', false)
...
# Ecart brut d'Habilete (Billy - adversaire), AVANT le clampi([-7,7]) de
# clamp_diff() -- celui-la sert a LIRE la Table des Situations (qui ne
# couvre que cette plage), pas a savoir si la situation REELLE la depasse.
func get_ecart_brut() -> int:
	return self.hab_billy - self.hab_adversaire


# Vrai quand l'ecart depasse la derniere ligne de la table (+7) :
# situation si favorable a Billy qu'il n'y a plus besoin de jouer un seul
# tour pour savoir qui gagne. Volontairement ASYMETRIQUE -- pas de
# "defaite automatique" equivalente sous -7 (rien de source pour ca) :
# sous -7, le tour se joue quand meme sur la ligne -7, seul
# is_ecart_plafonne() previent que la realite est plus severe que ce qui
# s'affiche.
func is_auto_win() -> bool:
	return self.get_ecart_brut() > 7


# Vrai quand l'ecart reel (dans un sens ou l'autre) depasse ce que couvre
# la table -- l'UI doit le dire, sinon la situation affichee (bornee a la
# derniere ligne couverte) donne l'impression d'un bug plutot que d'etre
# juste plus extreme que ce qui est montre.
func is_ecart_plafonne() -> bool:
	var ecart_brut = self.get_ecart_brut()
	return ecart_brut < -7 or ecart_brut > 7
...
# Modificateurs), jet d'attaque (relance automatique du DEBROUILLARD si
# applicable, cf plus bas), esquive adverse eventuelle, esquive de Billy
# si son ADRESSE le permet, application de l'Armure et du plafond de
# degats subis (PAYSAN), puis effets apres-tour (brasiers periodiques,
# etc.). Les trois jets sont optionnels (roll_die() par defaut) pour
...
#
# attack_die_roll_relance : le SECOND de d'attaque du DEBROUILLARD, ignore
# si peut_relancer_attaque est faux. Un vrai relancer garde toujours le
# MEILLEUR des deux (jamais un remplacement qui pourrait donner pire) --
# et se joue automatiquement (pas un choix du joueur, cf decision produit :
# relancer ne peut jamais desservir Billy, inutile de lui faire cliquer un
# bouton pour un choix qui n'en est pas vraiment un).
func play_turn(attack_die_roll = null, esquive_die_roll = null, attack_die_roll_relance = null) -> EtatTour:
	if self.is_over():
		push_error("Combat.play_turn: le combat est deja termine")
		return null
	if attack_die_roll == null:
		attack_die_roll = roll_die()
	if self.peut_relancer_attaque:
		if attack_die_roll_relance == null:
			attack_die_roll_relance = roll_die()
		attack_die_roll = maxi(attack_die_roll, attack_die_roll_relance)
