...
var _pv_minus_button: Button
var _pv_plus_button: Button
var _cha_bar_fill: ColorRect
var _cha_value_field: LineEdit
var _cha_minus_button: Button
var _cha_plus_button: Button
...
func _build_pv_card() -> PanelContainer:
	...
	self._pv_minus_button = self._make_step_button("−", true)
	self._pv_minus_button.pressed.connect(self._step_pv.bind(-1))
	bar_row.add_child(self._pv_minus_button)
	...
	self._pv_plus_button = self._make_step_button("+", false)
	self._pv_plus_button.pressed.connect(self._step_pv.bind(1))
	bar_row.add_child(self._pv_plus_button)
...
func _build_cha_strip() -> HBoxContainer:
	...
	self._cha_minus_button = self._make_step_button("−", true)
	self._cha_minus_button.pressed.connect(self._step_cha.bind(-1))
	row.add_child(self._cha_minus_button)
	...
	self._cha_plus_button = self._make_step_button("+", false)
	self._cha_plus_button.pressed.connect(self._step_cha.bind(1))
	row.add_child(self._cha_plus_button)
...
func refresh() -> void:
	...
	# Un bouton +/- qui ne peut plus rien faire (deja a sa borne) est grise --
	# sinon il a l'air casse plutot qu'inutile a cet instant. S'ajoute au
	# verrou combat ci-dessus, ne le remplace pas (or logique : combat OU borne).
	self._pv_minus_button.disabled = self._pv_minus_button.disabled or Player.pv <= 0
	self._pv_plus_button.disabled = self._pv_plus_button.disabled or Player.pv >= Player.pv_max
	self._cha_minus_button.disabled = self._cha_minus_button.disabled or Player.cha <= 0
	self._cha_plus_button.disabled = self._cha_plus_button.disabled or Player.cha >= Player.chamax
